...
func SeedMySQLDataModels(ctx context.Context, engine *project.Engine) (map[string][]*model.DataModel, error) {
	...
	if _, err := SeedMySQLModelConfigs(ctx, engine); err != nil {
	...
	for _, modelID := range modelOrder {
		...
		if modelID != "address" {
			log.Printf("[SEED] [Schema] Applying & compiling MySQL DDL migration for model '%s'...", modelID)
			if _, applyErr := engine.ApplySchema(ctx, modelID, service.ApplyRequest{}); applyErr != nil {
				...
				log.Printf("[SEED] [Schema] ✔ MySQL schema applied and active for '%s'", modelID)
			}
		}
	}
...
func SeedMySQLSampleData(ctx context.Context, engine *project.Engine) (map[string]any, error) {
	...
	empData := map[string]any{
		...
		"skills":             []any{"Go", "Distributed Systems", "MySQL"},
...
func SeedEnterpriseMySQLSchema(ctx context.Context, engine *project.Engine) (map[string]any, error) {
	...
	modelConfigs, err := SeedMySQLModelConfigs(ctx, engine)
	if err != nil {
		return nil, err
	}
	...
	sampleRecords, err := SeedMySQLSampleData(ctx, engine)
	if err != nil {
		return nil, err
	}

	log.Println("[SEED] =========================================================")
	log.Println("[SEED] ✔ Enterprise MySQL Schema Seeding Completed Successfully!")
	log.Println("[SEED] =========================================================")

	return map[string]any{
		"status":        "SUCCESS",
		"message":       "Successfully seeded 5 tables/models and 4 references with complete validations and records in MySQL!",
